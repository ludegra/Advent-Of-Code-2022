...
fn solve(_input: impl Iterator<Item = String>, start: Instant) {
    ...
    let max = monkies
        .iter()
        .fold(1, |acc, (_, monke)| acc * monke.test_value);
    ...
    for _ in 0..10000 {
        for key in 0..monkies.keys().len() {
            ...
            while !current_monkey.items.is_empty() {
                ...
                let mut new_fear_level = match current_monkey.operation {
    ...
    let product = sorted.next().unwrap() as u128 * sorted.next().unwrap() as u128;
